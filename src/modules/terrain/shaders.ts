...
void tClass(int cls, float orient, vec2 xz, float lush, float vn, float dist,
  ...
  if (wA < 0.995 && lb != la) {
    ...
    float k = tHeightBlend(wA, nA.z, nB.z);
    alb = mix(aB, alb, k); nrm = mix(nB, nrm, k);
    rough = mix(uLayerP[lb].y, rough, k);
    mean = mix(uLayerMean[lb], mean, wA);
  }
  if (E.x > 0.0 && lc != la) {
...
export const FRAG_MAIN = /* glsl */ `
{
  ...
  if (detail) {
    ...
    if (ib >= 0 && wb > 0.02) {
      ...
      float k = tHeightBlend(wa / (wa + wb), nrmT.z, nrmB.z);
  ...
  float tintK = mix(uLook.x, 1.0, smoothstep(20.0, 400.0, dist));
  ...
  float dn = (1.0 - fade) * (detail ? 1.0 : 0.0);
