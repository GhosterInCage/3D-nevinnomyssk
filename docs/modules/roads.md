...
| Parking lots | Overture `infrastructure` parking polygons | asphalt |
...
| Railways | `rail` segments (standard gauge 1520 mm); `is_disused` = rusty rails and wooden sleepers; `is_abandoned` not drawn | ballast bed in the ground mesh (sloped shoulders), 3D rails + sleepers streamed in 150 m chunks within ~420 m, level crossings with rails flush to the road, low platforms (0.95 m) |
...
| Street lights | LED steel poles (both sides of main roads), sodium lamps on concrete poles (tertiary, apartment districts), distribution poles with lamps + overhead SIP cables in the private sector, lamps on bridge parapets | instanced within ~1 km; glowing lenses at night; night glow sprites for every lamp in the city; lamp light pools on all road surfaces |
...
* **One PBR surface shader** (`materials.ts`, MeshStandardMaterial + `onBeforeCompile`) for all hard
  ...
  crumbling edges on narrow old roads, a grass strip in the middle of dirt tracks, mid-distance track pattern
...
roads.graph.edges[i]         // { a, b, cls, width, lanes, oneway, dir (+1 a->b only, -1 b->a only, 0 both), name?,
                             ...
                             //   points: Float32Array x,z, y: Float32Array surface height per point (deck on bridges) }
